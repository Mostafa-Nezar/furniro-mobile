...
const ProfileHeader = ({ user, theme, cart, favorites, isUploading, onPickImage }) => (
  <View style={[tw`p-6 items-center`, { backgroundColor: theme.lightBeige }]}>
    ...
    <View style={[tw`w-3/4 flex-row mt-4 justify-evenly`]}>
      <View style={tw`flex-row items-center`}>
        <Icon name="shopping-cart" size={32} color={theme.primary} />
        <Text style={[tw`text-lg font-bold ml-2`, { color: theme.primary }]}>{cart.length}</Text>
      </View>
      <View style={tw`flex-row items-center`}>
        <Icon name="favorite" size={32} color={theme.primary} />
        <Text style={[tw`text-lg font-bold ml-2`, { color: theme.primary }]}>{favorites.length}</Text>
      </View>
    </View>
  </View>
);
