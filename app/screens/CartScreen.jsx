...
const CartScreen = () => {
  ...
  const renderItem = ({ item }) => (
    <View style={[tw`flex-row p-4 mb-3 rounded-lg`, { backgroundColor: theme.semiWhite }]}>
      <Image source={{ uri: getImage(item.image) }} style={tw`w-20 h-20 rounded-lg`} />
      <View style={tw`flex-1 ml-4`}>
        ...
        <View style={tw`flex-row justify-between items-center mt-2`}>
          ...
          <View style={tw`flex-row items-center`}>
            <TouchableOpacity onPress={() => handleQuantityChange(item.id, item.quantity - 1)} style={[tw`w-8 h-8 rounded-full items-center justify-center`, { backgroundColor: theme.lightGray }]}>
              <Icon name="remove" size={16} color={theme.black} />
            </TouchableOpacity>
            <Text style={[tw`mx-3`, { color: theme.black }]}>{item.quantity}</Text>
            <TouchableOpacity onPress={() => handleQuantityChange(item.id, item.quantity + 1)} style={[tw`w-8 h-8 rounded-full items-center justify-center`, { backgroundColor: theme.primary }]}>
              <Icon name="add" size={16} color={theme.white} />
            </TouchableOpacity>
          </View>
        </View>
      </View>
      <TouchableOpacity onPress={() => handleQuantityChange(item.id, 0)} style={tw`ml-2 p-2`}>
  ...
  return (
    <View style={[tw`flex-1`, { backgroundColor: theme.white }]}>
      ...
      <View style={[tw`p-4 border-t`, { borderTopColor: theme.lightGray }]}>
        ...
        <TouchableOpacity onPress={() => Alert.alert('Order Placed')} style={[tw`py-4 rounded-lg mt-4`, { backgroundColor: theme.primary }]}>
